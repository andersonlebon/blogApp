...
RSpec.describe User, type: :model do
  ...
  # test if user is not valid without a bio
  it 'is not valid without a bio' do
    @user.bio = nil
    expect(@user).to_not be_valid
  end
  # test if user is not valid without a photo
  it 'is not valid without a photo' do
    @user.photo = nil
    expect(@user).to_not be_valid
  end
  # test if user is not valid with a name longer than 50 characters
  it 'is not valid with a name longer than 50 characters' do
    @user.name = 'a' * 51
    expect(@user).to_not be_valid
  end
  # test if user is not valid with a bio longer than 255 characters
  it 'is not valid with a bio longer than 255 characters' do
    @user.bio = 'a' * 256
    expect(@user).to_not be_valid
  end
  # test if user is not valid with a photo longer than 255 characters
